...
def transformToRSpace(px,py,a,Rx,Ry):
	nx = len(Rx)
	ny = len(Ry)
	Nx = len(px)
	Ny = len(py)
	...
	print ("Generating unitary transform")
	U = np.zeros((nx,ny,N/2) , dtype = complex)
		...
		for iY in range(Ny):
			for x in range(nx):
				for y in range(ny):
					...
					x_j = Rx[x]
					y_j = Ry[y]
					pr = px_i * x_j + py_i * y_j
					U[x,y,i] = math.cos(pr) + 1j*math.sin(pr)

	print("Transforming")

	psi3 = np.zeros((N,nx,ny), dtype=complex)
	psi4 = np.zeros((N,nx,ny), dtype=complex)
...
def makeLDOS(psi3, psi4, probDensity, E, Eval, gamma, nx, ny):
	LDOS = np.zeros((nx,ny))

	print("Calculating LDOS")
	for n in range(len(E)):
		LDOS += probDensity[n] / ((Eval - E[n])**2 + gamma**2)
...
def makeFilename2(alpha, Nx, Ny, eta, h, Eval):
	fname = 'data2-alpha=%g-N=%dx%d-eta=%g-h=%g-epsilon=%g.npz' % (alpha, Nx, Ny, eta, h, Eval)
	return fname
...
def solveAndSave(Nx, Ny, pxmax, pymax, alpha, h, eta):
	...
	print ("done")
	f = makeFilename(alpha, Nx, Ny, eta, h)
